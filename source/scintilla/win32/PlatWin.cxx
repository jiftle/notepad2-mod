...
//TCHAR * uCharNext(const TCHAR * lpsz)
//{
//	return CharNext(lpsz);
//}
//
//TCHAR * uCharPrev(const TCHAR * lpszStart, const TCHAR * lpszCurrent)
//{
//	return CharPrev(lpszStart, lpszCurrent);
//}
...
void Palette::Allocate(Window &) {
	...
	if (allowRealization) {
		BYTE *pal = new BYTE[sizeof(LOGPALETTE) + (used-1) * sizeof(PALETTEENTRY)];
...
static void SetLogFont(LOGFONT &lf, const tchar *faceName, int characterSet, int size, bool bold, bool italic) {
	...
	_tcsncpy(lf.lfFaceName, faceName, sizeof(lf.lfFaceName));
}
 ...
 */
static int HashFont(const tchar *faceName, int characterSet, int size, bool bold, bool italic) {
	return
		size ^
		(characterSet << 10) ^
		(bold ? 0x10000000 : 0) ^
...
class FontCached : Font {
	...
	LOGFONT lf;
	int hash;
	FontCached(const tchar *faceName_, int characterSet_, int size_, bool bold_, bool italic_);
	~FontCached() {}
	bool SameAs(const tchar *faceName_, int characterSet_, int size_, bool bold_, bool italic_);
	virtual void Release();

	static FontCached *first;
public:
	static FontID FindOrCreate(const tchar *faceName_, int characterSet_, int size_, bool bold_, bool italic_);
	static void ReleaseId(FontID id_);
};

FontCached *FontCached::first = 0;

FontCached::FontCached(const tchar *faceName_, int characterSet_, int size_, bool bold_, bool italic_) :
...
bool FontCached::SameAs(const tchar *faceName_, int characterSet_, int size_, bool bold_, bool italic_) {
	return
		...
		0 == _tcscmp(lf.lfFaceName,faceName_);
}
...
FontID FontCached::FindOrCreate(const tchar *faceName_, int characterSet_, int size_, bool bold_, bool italic_) {
	FontID ret = 0;
...
void Font::Create(const tchar *faceName, int characterSet, int size,
...
public:
	...
	void DrawTextCommon(PRectangle rc, Font &font_, int ybase, const tchar *s, int len, UINT fuOptions);
	void DrawTextNoClip(PRectangle rc, Font &font_, int ybase, const tchar *s, int len, ColourAllocated fore, ColourAllocated back);
	void DrawTextClipped(PRectangle rc, Font &font_, int ybase, const tchar *s, int len, ColourAllocated fore, ColourAllocated back);
	void DrawTextTransparent(PRectangle rc, Font &font_, int ybase, const tchar *s, int len, ColourAllocated fore);
	void MeasureWidths(Font &font_, const tchar *s, int len, int *positions);
	int WidthText(Font &font_, const tchar *s, int len);
	int WidthChar(Font &font_, tchar ch);
...
void SurfaceImpl::DrawTextCommon(PRectangle rc, Font &font_, int ybase, const char *s, int len, UINT fuOptions) {
	...
	RECT rcw = RectFromPRectangle(rc);
#ifdef UNICODE
	::ExtTextOut(hdc, rc.left, ybase, fuOptions, &rcw, s,
...
void SurfaceImpl::DrawTextNoClip(PRectangle rc, Font &font_, int ybase, const tchar *s, int len,
...
void SurfaceImpl::DrawTextClipped(PRectangle rc, Font &font_, int ybase, const tchar *s, int len,
	ColourAllocated fore, ColourAllocated back) {
	::SetTextColor(hdc, fore.AsLong());
	::SetBkColor(hdc, back.AsLong());
...
void SurfaceImpl::DrawTextTransparent(PRectangle rc, Font &font_, int ybase, const tchar *s, int len,
...
int SurfaceImpl::WidthText(Font &font_, const tchar *s, int len) {
	SetFont(font_);
	SIZE sz={0,0};
#ifdef UNICODE
...
void SurfaceImpl::MeasureWidths(Font &font_, const tchar *s, int len, int *positions) {
	SetFont(font_);
	SIZE sz={0,0};
	int fit = 0;
#ifdef UNICODE
...
int SurfaceImpl::WidthChar(Font &font_, tchar ch) {
	SetFont(font_);
	SIZE sz;
...
void Window::SetTitle(const tchar *s) {
...
private:
	...
	tchar *AllocWord(const tchar *word);
...
public:
	...
	ListItemData *Append(const tchar *text, int value);
	...
	void SetWords(tchar *s) {
		words = s;	// N.B. will be deleted on destruction
	}
};

tchar *LineToItem::AllocWord(const tchar *text) {
	int chars = _tcslen(text) + 1;
	int newCount = wordsCount + chars;
	if (newCount > wordsSize) {
		wordsSize = _ROUND2(newCount * 2, 8192);
		tchar *wordsNew = new tchar[wordsSize];
		memcpy(wordsNew, words, wordsCount * sizeof(tchar));
	...
	tchar *s = &words[wordsCount];
	wordsCount = newCount;
	_tcsncpy(s, text, chars);
	return s;
}
...
ListItemData *LineToItem::Append(const tchar *text, int imageIndex) {
...
class ListBoxX : public ListBox {
	...
	HWND GetHWND() const;
	void AppendListItem(const tchar *startword, const tchar *numword);
...
public:
	...
	virtual void Clear();
	virtual void Append(tchar *s, int type = -1);
	virtual int Length();
	virtual void Select(int n);
	virtual int GetSelection();
	virtual int Find(const tchar *prefix);
	virtual void GetValue(int n, tchar *value, int len);
	...
	virtual void SetList(const tchar *list, tchar separator, tchar typesep);
...
PRectangle ListBoxX::GetDesiredRect() {
	...
	SIZE textSize = {0, 0};
	int len = widestItem ? _tcslen(widestItem) : 0;
#ifdef UNICODE
...
int ListBoxX::Find(const tchar *) {
	return LB_ERR;
}

void ListBoxX::GetValue(int n, tchar *value, int len) {
	ListItemData item = lti.Get(n);
	_tcsncpy(value, item.text, len);
	value[len-1] = '\0';
}
