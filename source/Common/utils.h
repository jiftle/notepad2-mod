...
//  ～(灬ミ^ō^ミ^ō^ミ灬)～ 
// 基于 STL 的一些补充
// 由于 Visual C++ 6.0 不符合 C++ 标准，因此 Visual C++ 6.0 中无法通过编译
//  -------------------------------------------------------------
...
#pragma warning(disable:4505) // Message: unreferenced local function has been removed
#endif
#ifdef _WIN32
#ifndef NOMINMAX
#define NOMINMAX
...
#define WPF_NO_VTABLE __declspec(novtable)
...
{ return io.erase(util::min(io.find_last_not_of(elem) + 1, io.length())); }
...
public:
	...
	inline T * operator()() { return xm_data; }
	inline const T * operator()() const { return xm_data; }
...
public:
	...
	inline bool operator== (const format & rhs) { return !strcmp(buf, rhs.buf); }
...
private:
	format & _copy(const format & rhs)
	{
		...
		{
			delete [] buf;
			buf = new TCHAR[strlen(rhs.buf) + 1];
			strcpy(buf, rhs.buf);
		}
		return *this;
	}
