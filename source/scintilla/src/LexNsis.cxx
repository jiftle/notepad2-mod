...
static bool isNsisHexNumber(char ch)
{
...
{
  return (ch == '.' ) || (ch == '_' ) || (ch == '|') || isNsisNumber(ch) || (ch >= 'A' && ch <= 'Z') || (ch >= 'a' && ch <= 'z');
}
...
static int classifyWordNsis(unsigned int start, unsigned int end, WordList *keywordLists[], Accessor &styler )
{
  ...
  if( NsisCmp(s, "PageEx", bIgnoreCase) == 0 || NsisCmp(s, "PageExEnd", bIgnoreCase) == 0 ) // Covers PageEx and PageExEnd
	...
	if( s_len > 3 )
	{
		if(( s[1] == '{' && s[s_len-1] == '}' ) || ( s[1] == '(' && s[s_len-1] == ')' ))
			return SCE_NSIS_VARIABLE;
	}
  ...
  {
	  bool bHasSimpleNsisNumber = true;
	  for (unsigned int j = 1; j < end - start + 1 && j < 11; j++)
	  {
		  if( s[j] == '\0' || s[j] == '\r' || s[j] == '\n' )
			  break;
		  if( !isNsisHexNumber( s[j] ) && !isNsisNumber( s[j] ) )
		  {
			  bHasSimpleNsisNumber = false;
			  break;
		  }
  ...
  bool bClassicVarInString = false;
	...
	for( i = startPos; i < nLengthDoc; i++ )
	{
		...
		switch(state)
		{
          ...
          if( ( isNsisNumber(cCurrChar) || isNsisHexNumber(cCurrChar) ) && (cNextChar == '\t' || cNextChar == ' ' || cNextChar == '\r' || cNextChar == '\n' ) )
