 ...
 */
bool Document::InsertString(int position, const tchar *s) {
	return InsertString(position, s, _tcslen(s));
}

/**
 * Insert a string with a length.
 */
bool Document::InsertString(int position, const tchar *s, size_t insertLength) {
	bool changed = false;
	if (insertLength > 0) {
		tchar *sWithStyle = new tchar[insertLength * 2];
		if (sWithStyle) {
