...
static bool IsArrowCharacter(tchar ch) {
	return (ch == 0) || (ch == '\001') || (ch == '\002');
}

void CallTip::DrawChunk(Surface *surface, int &x, const tchar *s,
...
int CallTip::PaintContents(Surface *surfaceWindow, bool draw) {
	...
	int maxWidth = 0;
	while (moreChunks) {
		tchar *chunkEnd = _tcschr(chunkVal, '\n');
		if (chunkEnd == NULL) {
			chunkEnd = chunkVal + _tcslen(chunkVal);
			moreChunks = false;
		}
...
PRectangle CallTip::CallTipStart(int pos, Point pt, const tchar *defn,
                                 const tchar *faceName, int size,
                                 int codePage_, int characterSet, Window &wParent) {
	clickPlace = 0;
	if (val)
		delete []val;
	val = new tchar[_tcslen(defn) + 1];
	if (!val)
		return PRectangle();
	_tcscpy(val, defn);
	codePage = codePage_;
	...
	int numLines = 1;
	const tchar *newline;
	const tchar *look = val;
	rectUp = PRectangle(0,0,0,0);
	rectDown = PRectangle(0,0,0,0);
	offsetMain = 5;
	int width = PaintContents(surfaceMeasure, false) + 5;
	while ((newline = _tcschr(look, '\n')) != NULL) {
		look = newline + 1;
		numLines++;
	}
