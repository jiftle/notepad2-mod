...
void Editor::LayoutLine(int line, Surface *surface, ViewStyle &vstyle, LineLayout *ll, int width) {
	if (!ll || line >= pdoc->LinesTotal())
		return;
	PLATFORM_ASSERT(line < pdoc->LinesTotal());
	//if (line >= pdoc->LinesTotal()) line = pdoc->LinesTotal() - 1;
...
#ifdef _WIN32
					// process English mix Chinese wrap
					else if ( ((unsigned short)ll->chars[p - 1] < 0x7F && (unsigned short)ll->chars[p] >= 0x7F)
						|| ((unsigned short)ll->chars[p - 1] >= 0x7F && (unsigned short)ll->chars[p] < 0x7F) )
						lastGoodBreak = p;
					else if ((unsigned short)ll->chars[p - 1] >= 0x7F && (unsigned short)ll->chars[p] >= 0x7F)
					{
						TCHAR * start = ll->chars + p;
						TCHAR * end = CharPrev(ll->chars, start);
						end = CharNext(end);
						lastGoodBreak = p + end - start;
					}
#endif
				}
				p++;
...
inline tchar * CharPtrFromSPtr(sptr_t lParam) {
	return reinterpret_cast<tchar *>(lParam);
}
