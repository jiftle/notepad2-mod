...
string ASFormatter::nextLine()
{
    ...
    while (!isLineReady)
    {
        ...
        if (!isInTemplate)
        {
            if ( (newHeader = findHeader(headers)) != NULL)
            {
                ...
                goForward((int)currentHeader->length() - 1);
            ...
            else if ( (newHeader = findHeader(preDefinitionHeaders)) != NULL)
            {
                ...
                goForward((int)newHeader->length() - 1);
    ...
    int readyFormattedLineLength = (int)trim(readyFormattedLine).length();
...
bool ASFormatter::getNextChar()
{
    ...
    if (shouldPadOperators && !isInComment && !isInLineComment
            ...
            && !isBeforeComment())
    {
        int len = (int)formattedLine.length();
    ...
    int currentLineLength = (int)currentLine.length();
...
{
    int formattedLineLength = (int)formattedLine.length();
