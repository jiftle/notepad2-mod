...
string ASBeautifier::beautify(const string &originalLine)
{
    ...
    int spaceTabCount = 0;
    char tempCh;
    int headerStackSize = (int)headerStack->size();
    ...
    if (isCStyle && !isInComment && (line[0] == '#' || backslashEndsPrevLine))
    {
        if (line[0] == '#')
        {
            ...
            else if (preproc.COMPARE(0, 2, string("if")) == 0)
            {
                ...
                waitingBeautifierStackLengthStack->push_back((int)waitingBeautifierStack->size());
                activeBeautifierStackLengthStack->push_back((int)activeBeautifierStack->size());
    ...
    for (i=0; i<(int)line.length(); i++)
    {
        ...
        if (ch == '(' || ch == '[' || ch == ')' || ch == ']')
        {
            if (ch == '(' || ch == '[')
            {
                ...
                inStatementIndentStackSizeStack->push_back((int)inStatementIndentStack->size());
                ...
                else if (newHeader == &AS_ELSE)
                {
                    if (lastTempStack != NULL)
                    {
                        ...
                        if (indexOfIf != -1)
                        {
                            ...
                            int restackSize = (int)lastTempStack->size() - indexOfIf - 1;
                ...
                else if (newHeader == &AS_WHILE)
                {
                    if (lastTempStack != NULL)
                    {
                        ...
                        if (indexOfDo != -1)
                        {
                            ...
                            // from the temporary snapshot stored in lastTempStack.
                            int restackSize = (int)lastTempStack->size() - indexOfDo - 1;
                            for (int r=0; r<restackSize; r++)
                            {
                                headerStack->push_back(lastTempStack->back());
                                lastTempStack->pop_back();
                ...
                else if (newHeader == &AS_CATCH || newHeader == &AS_FINALLY)
                {
                    if (lastTempStack != NULL)
                    {
                        ...
                        if (indexOfTry != -1)
                        {
                            ...
                            // from the temporary snapshot stored in lastTempStack.
                            int restackSize = (int)lastTempStack->size() - indexOfTry - 1;
                            for (int r=0; r<restackSize; r++)
                            {
                                headerStack->push_back(lastTempStack->back());
                                lastTempStack->pop_back();
...
////        // to have assignment operators (here, '-=') in them . . .
		...
		if (foundNonAssignmentOp != NULL)
		{
			if (foundNonAssignmentOp->length() > 1)
            {
                ...
                i += (int)foundNonAssignmentOp->length() - 1;
            }
		}
		...
		else if (foundAssignmentOp != NULL)
		{
			if (foundAssignmentOp->length() > 1)
            {
                ...
                i += (int)foundAssignmentOp->length() - 1;
            }
        ...
        int minIndent, bool updateParenStack)
{
    ...
    int remainingCharNum = (int)line.length() - i;
    int nextNonWSChar = 1;
...
{
    int maxHeaders = (int)possibleHeaders.size();
    ...
    for (p=0; p < maxHeaders; p++)
    {
        ...
        if (line.COMPARE(i, header->length(), *header) == 0)
        {
            ...
            int headerEnd = i + (int)header->length();
