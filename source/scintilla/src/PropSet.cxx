...
static inline tchar MakeUpperCase(tchar ch) {
	if (ch < 'a' || ch > 'z')
		return ch;
	else
		return static_cast<tchar>(ch - 'a' + 'A');
}
...
int CompareCaseInsensitive(const tchar *a, const tchar *b) {
	while (*a && *b) {
		if (*a != *b) {
			tchar upperA = MakeUpperCase(*a);
			tchar upperB = MakeUpperCase(*b);
			if (upperA != upperB)
...
int CompareNCaseInsensitive(const tchar *a, const tchar *b, size_t len) {
	while (*a && *b && len) {
		if (*a != *b) {
			tchar upperA = MakeUpperCase(*a);
			tchar upperB = MakeUpperCase(*b);
...
bool EqualCaseInsensitive(const tchar *a, const tchar *b) {
