...
class ScintillaWin :
	...
	virtual void AddToPopUp(const tchar *label, int cmd = 0, bool enabled = true);
	...
	void AddCharBytes(tchar b0, tchar b1);
	...
	void InsertPasteText(const tchar *text, int len, int selStart, bool isRectangular);

public:
	// Public for benefit of Scintilla_DirectFunction
	virtual sptr_t WndProc(unsigned int iMessage, uptr_t wParam, sptr_t lParam);
	sptr_t WndProcSEH(unsigned int iMessage, uptr_t wParam, sptr_t lParam);
	...
	// External Lexers
#ifdef SCI_LEXER
	void SetLexerLanguage(const tchar *languageName);
...
//inline sptr_t ScintillaWin::WndProcSEH(unsigned int iMessage, uptr_t wParam, sptr_t lParam)
//{
//	sptr_t res = 0L;
//	//__TRY
//		res = this->WndProc(iMessage, wParam, lParam);
//	//__CATCH("Scintilla=>WndProc")
//	return res;
//}
...
void ScintillaWin::CopyToClipboard(const SelectionText &selectedText) {
	...
	if (IsUnicodeMode()) {
		...
		if (uniText) {
			::MultiByteToWideChar(CP_ACP, 0, selectedText.s, selectedText.len, 
